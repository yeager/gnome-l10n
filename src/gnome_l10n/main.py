...
class StatsRow(Gtk.Box):
    ...
    def __init__(self, stats: ModuleStats):
        ...
        detail = f"{stats.branch} / {stats.domain}"
        ...
        nums_box.set_size_request(200, -1)
...
class MainWindow(Adw.ApplicationWindow):
    def __init__(self, *args, **kwargs):
        ...
        # Controls bar: release + language dropdowns
        ...
        controls.append(self.filter_all)
        ...
        controls.append(self.filter_incomplete)
        ...
        controls.append(self.filter_complete)
    ...
    def _on_releases_loaded(self, releases):
        ...
        # Select gnome-49 (stable) by default
    ...
    def _on_stats_loaded(self, stats):
        self._stats = sorted(stats, key=lambda s: s.pct)
        self.spinner.stop()
        self.spinner.set_visible(False)
        self.progress_label.set_visible(False)
        self._update_view()
    ...
    def _update_view(self):
        ...
        self.summary_label.set_text(
            _("%d modules — %d complete") % (len(self._stats), complete)
        )
    ...
    def _on_filter_changed(self, btn, mode):
        if btn.get_active():
            ...
            # Deactivate others
            for b, m in [(self.filter_all, "all"), (self.filter_incomplete, "incomplete"), (self.filter_complete, "complete")]:
                if m != mode:
                    b.set_active(False)
            self._update_view()
    ...
    def _on_export(self, *_args):
        ...
        writer.writerow(["Module", "Branch", "Domain", "Translated", "Fuzzy", "Untranslated", "Total", "Percent"])
        for s in self._stats:
            writer.writerow([s.module, s.branch, s.domain, s.translated, s.fuzzy, s.untranslated, s.total, f"{s.pct:.1f}"])
        writer.writerow([])
...
# Need this import for export
from gnome_l10n.api import get_cache_dir
